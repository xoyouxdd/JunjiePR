...
async function renderSickLeaveImport(){
  ...
  const importTable=rows=>`<div class="table-wrap"><table><thead><tr><th>导入时间</th><th>员工</th><th>病假日期</th><th>类型</th><th>天数</th><th>状态</th></tr></thead><tbody>${rows.map(row=>`<tr class="${row.status==='covered'?'void-row':''}"><td>${esc(row.submitted_at)}</td><td>${esc(row.employee_name)}<br><small>${esc(row.employee_no)}</small></td><td>${esc(row.leave_start_date)} 至 ${esc(row.leave_end_date)}</td><td>${esc(row.leave_type)}</td><td>${Number(row.leave_days).toFixed(1)}天</td><td>${statusBadge(row)}${row.status==='covered'?`<br><small>${esc(row.void_reason)}</small>`:''}</td></tr>`).join('')}</tbody></table></div>`;
  const loadRecords=async()=>{const out=await api('/api/sick-leave-imports/records?'+new URLSearchParams({month:form.elements.month.value}));const rows=out.items||[],{visible,covered}=splitCoveredSickRecords(rows);records.innerHTML=rows.length?`${visible.length?importTable(visible):''}${coveredSickDisclosure(covered,importTable(covered))}`:'<div class="empty">所选月份暂无文件缺勤记录</div>';};
  form.elements.month.addEventListener('change',()=>{result.innerHTML='';loadRecords().catch(error=>toast(error.message,true));});
  form.onsubmit=async event=>{
    event.preventDefault();
    ...
    try{
      ...
      result.innerHTML=`${errors}<div class="notice"><strong>预检结果</strong><br>月份：${esc(out.month)}；文件匹配 ${out.matched_employee_count} 名员工、${out.matched_record_count} 条明细；当月旧记录 ${out.replaced_record_count} 条将被覆盖，其中 ${out.absent_employee_count} 名员工未在文件中出现，将视为无缺勤。</div>${protectedNote}${rows}${notImported.length?`<a class="secondary download-link" href="${portalPath('/api/sick-leave-imports/'+encodeURIComponent(out.token)+'/unmatched-file')}">下载未导入数据标红文件</a>`:''}${out.can_commit?'<div class="actions"><button id="commitSickLeaveImport" class="warn">确认覆盖所选月份全部缺勤</button></div>':''}`;
      document.getElementById('commitSickLeaveImport')?.addEventListener('click',async()=>{
        const confirmed=await confirmModal('确认覆盖整月缺勤',`<p>将以文件替换 ${esc(out.month)} 的全部旧缺勤，共 ${out.replaced_record_count} 条；文件未列出的 ${out.absent_employee_count} 名员工将视为本月无缺勤。</p><p>旧记录保留“已覆盖”审计状态，不再参与计天；LOA不变。</p>`,'确认覆盖整月');
        if(!confirmed)return;
        try{const committed=await api('/api/sick-leave-imports/commit',json('POST',{token:out.token,month:out.month}));result.innerHTML=`<div class="notice"><strong>覆盖完成</strong><br>${esc(committed.month)} 写入 ${committed.covered_record_count} 条文件明细；旧记录 ${committed.replaced_record_count} 条已覆盖，${committed.absent_employee_count} 名文件未列出的员工已清除本月缺勤计分；LOA保护跳过 ${committed.loa_protected_count} 条。</div>`;await loadRecords();toast('所选月份缺勤已覆盖并重算全勤分');}catch(error){toast(error.message,true);}
      });
    }catch(error){toast(error.message||'预检失败',true);}
  };
  await loadRecords();
}
