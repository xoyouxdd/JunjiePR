...
function trendResultsHtml(data){
  ...
  return `<section class="panel"><div class="statistics-page-heading"><h3>${esc(data.months[0])} 至 ${esc(data.end_month)} 趋势</h3><span class="field-hint">共 ${data.overall.length} 个月 · 计分口径与当月统计一致</span></div><div class="trend-cards">${cards}</div>${trendChartSvg(data.months,lines,(app.clientWidth||760)-(window.matchMedia('(max-width: 760px)').matches?26:38))}${trendLegendHtml(lines)}${trendTableHtml(data)}</section>`;
}
