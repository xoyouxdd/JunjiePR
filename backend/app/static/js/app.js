...
function menuItems() {
  ...
  if(has('SICK_LEAVE_IMPORT'))items.push(['sickLeaveImport','病假导入']);
...
const NAV_GROUP_DEFS=[
  ...
  {id:'data',label:'数据',ids:['statistics','prRankings','sickLeaveImport']},
...
function actionCenterDetailHtml(detail){
  ...
  if(detail.type==='backup_health')return `<section class="panel action-center-detail"><div class="statistics-detail-heading"><div><h3>${esc(detail.title)}</h3><span>最近检查：${esc(detail.checked_at_utc||'暂无')}</span></div><button type="button" class="secondary" data-close-action-detail>关闭</button></div><ul class="operations-issues">${rows.map(row=>`<li><strong>${esc(row.code)}</strong>${row.message?`：${esc(row.message)}`:''}</li>`).join('')||'<li>当前没有异常项</li>'}</ul></section>`;
...
async function renderActionCenter(){
  ...
  const todoCards=items.map(item=>`<article class="action-card severity-${esc(item.severity)}"><div class="action-card-main"><span class="action-severity">${item.severity==='critical'?'优先处理':item.severity==='warning'?'请跟进':'待核对'}</span><strong>${esc(item.title)}</strong><p>${esc(item.description)}</p></div><div class="action-card-side"><div class="action-count"><b>${Number(item.count)}</b><span>项</span></div><button type="button" class="secondary" data-action-tab="${esc(item.tab)}" data-action-type="${esc(item.type)}">立即查看</button></div></article>`).join('')||'<section class="empty action-center-empty"><strong>当前没有待处理事项</strong><span>新的待办出现后会在这里提醒你。</span></section>';
  ...
  app.querySelectorAll('[data-action-tab]').forEach(button=>button.onclick=async()=>{const detailTypes=new Set(['backup_health','ungrouped_employee','recognition_review','overdue_review']);if(!detailTypes.has(button.dataset.actionType)){state.tab=button.dataset.actionTab;renderTabs();render();return;}button.disabled=true;try{const detail=await api('/api/action-center/'+encodeURIComponent(button.dataset.actionType)+'/details');const host=document.getElementById('actionCenterDetail');host.innerHTML=actionCenterDetailHtml(detail);host.querySelector('[data-close-action-detail]')?.addEventListener('click',()=>{host.innerHTML='';});host.scrollIntoView({behavior:'smooth',block:'nearest'});}catch(error){toast(error.message,true)}finally{button.disabled=false;}});
...
async function renderSickLeaveImport(){
  if(!has('SICK_LEAVE_IMPORT')){app.innerHTML='<section class="panel"><div class="error">当前账号没有病假导入权限</div></section>';return;}
  app.innerHTML=`<section class="panel"><h2>病假事务导入</h2><p>上传 HR 导出的 .xls 文件。系统会先核对“事务”与“总数”区；仅替换此前由月度文件导入的同月记录，不覆盖LOA或人工登记。</p><form id="sickLeaveImportForm" class="form-stack" enctype="multipart/form-data"><label>员工事务文件<input name="workbook" type="file" accept=".xls" required><span class="field-hint">仅支持不超过 10 MB 的 .xls；源文件 ID 会去掉第一位后与系统 7 位员工号匹配。预检最多有效 20 分钟，失效后请重新上传。</span></label><button type="submit" class="primary">预检文件</button></form><div id="sickLeaveImportResult"></div></section>`;
  const form=document.getElementById('sickLeaveImportForm'),result=document.getElementById('sickLeaveImportResult');
  form.onsubmit=async event=>{event.preventDefault();const file=form.elements.workbook.files?.[0];if(!file){toast('请选择 .xls 文件',true);return;}const data=new FormData();data.set('workbook',file,file.name);try{const out=await api('/api/sick-leave-imports/preview',{method:'POST',body:data});const unmatched=out.unmatched||[],protectedRows=out.loa_protected||[],blocked=out.blocking_errors||[];const errors=blocked.length?`<div class="error"><strong>预检未通过</strong><ul>${blocked.map(item=>`<li>${esc(item)}</li>`).join('')}</ul></div>`:'';const rows=unmatched.length?`<div class="table-wrap"><table><thead><tr><th>行号</th><th>姓名</th><th>源ID</th><th>系统员工号</th><th>日期</th><th>原因</th></tr></thead><tbody>${unmatched.map(item=>`<tr><td>${item.row}</td><td>${esc(item.name)}</td><td>${esc(item.source_id)}</td><td>${esc(item.employee_no)}</td><td>${esc(item.date)}</td><td>${esc(item.reason)}</td></tr>`).join('')}</tbody></table></div>`:'<p class="field-hint">没有未匹配员工。</p>';const protectedNote=protectedRows.length?`<div class="notice"><strong>LOA保护</strong><br>${protectedRows.length} 条文件明细落在LOA日期内，已跳过，不会覆盖LOA记录。</div>`:'';result.innerHTML=`${errors}<div class="notice"><strong>预检结果</strong><br>月份：${esc(out.month||'—')}；可覆盖 ${out.matched_employee_count} 名员工、${out.matched_record_count} 条明细；未覆盖 ${unmatched.length} 条明细。</div>${protectedNote}${rows}${unmatched.length?`<a class="secondary download-link" href="${portalPath('/api/sick-leave-imports/'+encodeURIComponent(out.token)+'/unmatched-file')}">下载未覆盖人员标红文件</a>`:''}${out.can_commit?'<div class="actions"><button id="commitSickLeaveImport" class="warn">确认覆盖匹配员工当月病假</button></div>':''}`;document.getElementById('commitSickLeaveImport')?.addEventListener('click',async()=>{const confirmed=await confirmModal('确认覆盖月度病假',`<p>将替换 ${out.matched_employee_count} 名已匹配员工在 ${esc(out.month)} 的此前月度导入病假明细。</p><p>LOA和人工登记不会被修改。</p>`,'确认覆盖');if(!confirmed)return;try{const committed=await api('/api/sick-leave-imports/commit',json('POST',{token:out.token}));result.innerHTML=`<div class="notice"><strong>覆盖完成</strong><br>已覆盖 ${committed.covered_employee_count} 名员工、${committed.covered_record_count} 条病假明细；跳过 ${committed.unmatched_count} 条未匹配明细及 ${committed.loa_protected_count} 条LOA保护明细。</div>`;toast('月度病假已覆盖并重算全勤分');}catch(error){toast(error.message,true)}});}catch(error){toast(error.message||'预检失败',true)}};
}
...
async function renderLoa(){
  ...
  const load=async()=>{
    ...
    records.innerHTML=rows.length?`<div class="table-wrap"><table><thead><tr><th>员工</th><th>进入日期</th><th>结束日期</th><th>状态</th><th>备注</th><th>登记人</th><th>操作</th></tr></thead><tbody>${rows.map(row=>{
      const ongoing=!row.ends_on||row.ends_on>=today(),endControl=row.ends_on?esc(row.ends_on):`<div class="loa-record-end"><input type="date" value="${today()}" min="${esc(row.starts_on)}" aria-label="${esc(row.employee_name)}的LOA结束日期" data-loa-end-date="${row.id}"><button type="button" class="secondary" data-loa-complete="${row.id}" data-employee-id="${row.employee_id}" data-starts-on="${esc(row.starts_on)}" data-employee-name="${esc(row.employee_name)}">登记结束</button></div>`;
      return `<tr><td>${esc(row.employee_name)}<br><small>${esc(row.employee_no)}</small></td><td>${esc(row.starts_on)}</td><td>${endControl}</td><td><span class="badge ${ongoing?'warn':'ok'}">${ongoing?'LOA进行中':'已结束'}</span></td><td>${esc(row.note||'—')}</td><td>${esc(row.created_by_name)}</td><td><button type="button" class="secondary" data-loa-cancel="${row.id}">撤销</button></td></tr>`;
...
function trendDeltaHtml(delta,invert=false){if(!delta||Math.abs(delta.value)<0.005)return '<span class="trend-delta flat">与上月持平</span>';const up=delta.value>0,good=invert?!up:up;return `<span class="trend-delta ${good?'good':'bad'}">${up?'↑':'↓'}${fmt(Math.abs(delta.value))} 较上月</span>`;}
function trendChartSvg(months,lines){
  if(!months.length)return '<div class="empty">所选范围内没有数据</div>';
  const w=760,h=260,padL=52,padR=16,padT=18,padB=34,plotW=w-padL-padR,plotH=h-padT-padB;
  ...
  const labels=months.map((month,i)=>`<text x="${px(i).toFixed(1)}" y="${h-10}" text-anchor="middle" class="trend-axis">${esc(month.slice(2))}</text>`).join('');
...
function trendResultsHtml(data){
  ...
  return `<section class="panel"><div class="statistics-page-heading"><h3>${esc(data.months[0])} 至 ${esc(data.end_month)} 趋势</h3><span class="field-hint">共 ${data.overall.length} 个月 · 计分口径与当月统计一致</span></div><div class="trend-cards">${cards}</div>${trendChartSvg(data.months,lines)}${trendLegendHtml(lines)}${trendTableHtml(data)}</section>`;
}
