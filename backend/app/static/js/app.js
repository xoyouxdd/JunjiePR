...
const NAV_GROUP_DEFS=[
  ...
  {id:'data',label:'数据',ids:['statistics','prRankings','declarationStatistics','sickLeaveImport']},
...
async function renderSickLeaveImport(){
  ...
  app.innerHTML=`<div class="section-gap"><section class="panel"><h2>缺勤登记</h2><p>上传 HR 导出的 .xls 文件。系统会先核对“事务”与“总数”区；匹配员工当月的旧人工登记和旧文件记录均设为“已覆盖”，以本次文件为准。LOA记录不变，LOA日期内的文件明细仍会跳过。</p><form id="sickLeaveImportForm" class="form-stack" enctype="multipart/form-data"><label>员工事务文件<input name="workbook" type="file" accept=".xls" required><span class="field-hint">仅支持不超过 10 MB 的 .xls；源文件 ID 会去掉第一位后与系统 7 位员工号匹配。预检最多有效 20 分钟，失效后请重新上传。</span></label><button type="submit" class="primary">预检文件</button></form><div id="sickLeaveImportResult"></div></section><section class="panel"><h2>月度缺勤登记记录</h2><p class="field-hint">此处仅展示月度文件登记的病假；已覆盖的历史人工登记仍可在“我的登记记录”查看，但不再参与计天。</p><div id="sickLeaveImportRecords" class="empty">正在加载…</div></section></div>`;
  ...
  const loadRecords=async()=>{const out=await api('/api/sick-leave-imports/records');const rows=out.items||[];records.innerHTML=rows.length?`<div class="table-wrap"><table><thead><tr><th>导入时间</th><th>员工</th><th>病假日期</th><th>类型</th><th>天数</th><th>状态</th></tr></thead><tbody>${rows.map(row=>`<tr class="${row.status==='covered'?'void-row':''}"><td>${esc(row.submitted_at)}</td><td>${esc(row.employee_name)}<br><small>${esc(row.employee_no)}</small></td><td>${esc(row.leave_start_date)} 至 ${esc(row.leave_end_date)}</td><td>${esc(row.leave_type)}</td><td>${Number(row.leave_days).toFixed(1)}天</td><td>${statusBadge(row)}${row.status==='covered'?`<br><small>${esc(row.void_reason)}</small>`:''}</td></tr>`).join('')}</tbody></table></div>`:'<div class="empty">暂无月度病假导入记录</div>';};
  form.onsubmit=async event=>{event.preventDefault();const file=form.elements.workbook.files?.[0];if(!file){toast('请选择 .xls 文件',true);return;}const data=new FormData();data.set('workbook',file,file.name);try{const out=await api('/api/sick-leave-imports/preview',{method:'POST',body:data});const unmatched=out.unmatched||[],protectedRows=out.loa_protected||[],notImported=[...unmatched,...protectedRows],blocked=out.blocking_errors||[];const errors=blocked.length?`<div class="error"><strong>预检未通过</strong><ul>${blocked.map(item=>`<li>${esc(item)}</li>`).join('')}</ul></div>`:'';const rows=notImported.length?`<div class="table-wrap"><table><thead><tr><th>行号</th><th>姓名</th><th>源ID</th><th>系统员工号</th><th>日期</th><th>原因</th></tr></thead><tbody>${notImported.map(item=>`<tr><td>${item.row}</td><td>${esc(item.name)}</td><td>${esc(item.source_id)}</td><td>${esc(item.employee_no)}</td><td>${esc(item.date)}</td><td>${esc(item.reason)}</td></tr>`).join('')}</tbody></table></div>`:'<p class="field-hint">所有文件病假明细均可导入。</p>';const protectedNote=protectedRows.length?`<div class="notice"><strong>LOA保护</strong><br>${protectedRows.length} 条文件明细落在LOA日期内，已跳过，不会覆盖LOA记录。</div>`:'';result.innerHTML=`${errors}<div class="notice"><strong>预检结果</strong><br>月份：${esc(out.month||'—')}；可覆盖 ${out.matched_employee_count} 名员工、${out.matched_record_count} 条明细；未导入 ${notImported.length} 条明细。</div>${protectedNote}${rows}${notImported.length?`<a class="secondary download-link" href="${portalPath('/api/sick-leave-imports/'+encodeURIComponent(out.token)+'/unmatched-file')}">下载未导入数据标红文件</a>`:''}${out.can_commit?'<div class="actions"><button id="commitSickLeaveImport" class="warn">确认覆盖匹配员工当月病假</button></div>':''}`;document.getElementById('commitSickLeaveImport')?.addEventListener('click',async()=>{const confirmed=await confirmModal('确认覆盖月度病假',`<p>将以文件替换 ${out.matched_employee_count} 名已匹配员工在 ${esc(out.month)} 的旧人工及旧导入缺勤明细。</p><p>LOA不变；旧人工登记将标记为已覆盖，不再参与计天。</p>`,'确认覆盖');if(!confirmed)return;try{const committed=await api('/api/sick-leave-imports/commit',json('POST',{token:out.token}));result.innerHTML=`<div class="notice"><strong>覆盖完成</strong><br>已为 ${committed.covered_employee_count} 名员工写入 ${committed.covered_record_count} 条病假明细；原有 ${committed.replaced_record_count} 条记录已覆盖（其中人工登记 ${committed.replaced_manual_count} 条）；跳过 ${committed.unmatched_count} 条未匹配明细及 ${committed.loa_protected_count} 条LOA保护明细。</div>`;await loadRecords();toast('月度病假已覆盖并重算全勤分');}catch(error){toast(error.message,true)}});}catch(error){toast(error.message||'预检失败',true)}};
  await loadRecords();
}
...
function declarationRecordDetails(rows){
  ...
  const material=row=>row.document_url?`<button type="button" class="secondary declaration-material" data-file-preview="${esc(row.document_url)}" data-file-kind="pdf" data-file-title="${esc(row.employee_name)}的处分材料">查看材料</button>`:'<span class="field-hint">暂无可查看材料</span>';
