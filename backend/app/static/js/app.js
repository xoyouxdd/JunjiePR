...
async function renderAccountReset(){
  ...
  bindPasswordForm(document.getElementById('passwordPageForm'),form=>{toast('密码已更新');form.reset();form.querySelector('[name=current_password]')?.focus();form.querySelector('[name=new_password]').dispatchEvent(new Event('input'));});
  document.getElementById('resetEmployeePassword').onsubmit=async event=>{event.preventDefault();const body=Object.fromEntries(new FormData(event.target));if(!confirm(`确认重置 ${body.name}（${body.employee_no}）的密码吗？原登录会话将立即失效，密码将重置为登录账号后四位。`))return;try{const out=await api('/api/accounts/reset-password',json('POST',body));document.getElementById('resetPasswordResult').innerHTML=`<div class="notice"><strong>${esc(out.employee_name)}</strong> 的重置密码：<code>${esc(out.temporary_password)}</code><br>密码为登录账号后四位；首次登录后必须修改。${out.account_enabled?'':' 该账号当前处于停用状态。'}</div>`;toast('密码已重置为登录账号后四位');event.target.reset()}catch(error){toast(error.message,true)}};
