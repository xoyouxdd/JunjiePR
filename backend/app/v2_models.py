...
class SickLeaveRecord(Base):
    ...
    proof_file_id: Mapped[int | None] = mapped_column(ForeignKey("stored_files.id"), nullable=True)
    leave_type: Mapped[str] = mapped_column(String(30), default="病假", index=True)
    import_source: Mapped[str] = mapped_column(String(30), default="manual", index=True)
...
class DeductionMaterialJob(Base):
    ...
    source_cleanup_status: Mapped[str] = mapped_column(String(20), default="idle")
    source_cleanup_attempts: Mapped[int] = mapped_column(Integer, default=0)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.now, index=True)
