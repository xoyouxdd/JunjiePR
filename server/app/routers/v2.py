...
from app.push_notifications import public_vapid_key, send_notification_pushes
...
def month_close_checklist(db: Session, month: str, attraction_id: int) -> list[dict]:
    """Only unresolved work that can alter this month blocks the circle close."""
    month_start = date.fromisoformat(f"{month}-01")
    month_end = (month_start.replace(day=28) + timedelta(days=4)).replace(day=1)
    employee_ids = [row[0] for row in db.query(Employee.id).filter(Employee.attraction_id == attraction_id).all()]
    pending_recognitions = db.query(RecognitionRecord).filter(
        RecognitionRecord.home_attraction_id == attraction_id,
    ...
    pending_transfers = db.query(CircleTransferRequest).filter(
...
def month_closure_payload(db: Session, month: str, attraction_id: int | None, user: V2User) -> dict:
    ...
    return {
        ...
        "checklist": month_close_checklist(db, month, attraction_id) if attraction_id is not None else [],
    }
...
def existing_submission(db: Session, actor_id: int, operation: str, request_key: str | None, model):
    if not request_key:
        return None
    ...
    return db.get(model, request_row.entity_id) if request_row else None


def remember_submission(db: Session, actor_id: int, operation: str, request_key: str | None, entity_id: int) -> None:
    if request_key:
        db.add(SubmissionRequest(actor_id=actor_id, operation=operation, request_key=request_key, entity_id=entity_id))
        db.flush()
...
def employee_targets(
    ...
    global_target_search = usage in {"deduction", "sick_leave"} and user.role.code in {"SUPERVISOR", "TA_GSM", "GSM"}
    ...
    if user.role.code == "TA_SUPERVISOR" and usage in {"deduction", "sick_leave"}:
        attraction_id = user.employee.attraction_id
    if attraction_id is not None:
        circle = db.get(Attraction, attraction_id)
        if not circle or not circle.active or not circle.employee_circle:
...
async def create_recognition(
    ...
    try:
        ...
        if is_self and image:
            ...
            db.flush()
        remember_submission(db, user.id, "recognition", request_key, row.id)
...
def notification_payload(row: AppNotification) -> dict:
    return {
        "id": row.id,
...
def my_notifications(limit: int = 30, db: Session = Depends(get_db), user: V2User = Depends(current_user)):
    limit = max(1, min(int(limit or 30), 100))
    rows = (
        db.query(AppNotification)
        .filter(AppNotification.employee_id == user.id)
        .order_by(AppNotification.created_at.desc())
        .limit(limit)
        .all()
    )
    unread = db.query(func.count(AppNotification.id)).filter(
        AppNotification.employee_id == user.id,
        AppNotification.read_at.is_(None),
    ).scalar() or 0
    return {"items": [notification_payload(row) for row in rows], "unread_count": int(unread)}


@router.post("/notifications/read")
def read_notifications(payload: dict, db: Session = Depends(get_db), user: V2User = Depends(current_user)):
    raw_ids = payload.get("ids") if isinstance(payload, dict) else None
    ids = [int(value) for value in (raw_ids or []) if str(value).isdigit()][:100]
    if ids:
        db.query(AppNotification).filter(
            AppNotification.employee_id == user.id,
            AppNotification.id.in_(ids),
            AppNotification.read_at.is_(None),
        ).update({AppNotification.read_at: datetime.now()}, synchronize_session=False)
        db.commit()
    return {"ok": True}
...
def action_center(db: Session = Depends(get_db), user: V2User = Depends(current_user)):
    ...
    if role_code in FRONTLINE_CODES:
        unread = db.query(AppNotification).filter(
            AppNotification.employee_id == user.id,
            AppNotification.read_at.is_(None),
        ).count()
        if unread:
            items.append(action_center_item("approval_notification", "待查看审批结果", unread, "home", "info", "签卡批复已写入站内消息。"))
...
def notification_push_config(user: V2User = Depends(current_user)):
    try:
        return {"enabled": True, "public_key": public_vapid_key()}
    except Exception:
        # The normal approval workflow remains available when server push setup is unavailable.
        raise HTTPException(503, "系统外提醒暂时不可用，请稍后重试")


@router.post("/push-subscriptions")
def save_push_subscription(payload: dict, request: Request, db: Session = Depends(get_db), user: V2User = Depends(current_user)):
    subscription = payload.get("subscription") if isinstance(payload, dict) else None
    if not isinstance(subscription, dict):
        raise HTTPException(400, "浏览器通知授权信息无效，请重新开启提醒")
    endpoint = str(subscription.get("endpoint") or "").strip()
    keys = subscription.get("keys") or {}
    p256dh = str(keys.get("p256dh") or "").strip()
    auth = str(keys.get("auth") or "").strip()
    if not endpoint.startswith("https://") or len(endpoint) > 4096 or not p256dh or not auth or len(p256dh) > 255 or len(auth) > 255:
        raise HTTPException(400, "浏览器通知授权信息无效，请重新开启提醒")
    row = db.query(PushSubscription).filter(PushSubscription.endpoint == endpoint).first()
    if not row:
        row = PushSubscription(employee_id=user.id, endpoint=endpoint, p256dh=p256dh, auth=auth)
        db.add(row)
    else:
    ...
    write_audit(db, user.employee, "开启系统外提醒", "push_subscription", after={"active": True}, ip_address=client_ip(request))
    db.commit()
...
def review_recognition(record_id: int, payload: dict, request: Request, db: Session = Depends(get_db), user: V2User = Depends(require_permissions("REVIEW_DIRECT"))):
    ...
    if action in {"confirm", "reject"}:
        passed = action == "confirm"
        notification = AppNotification(
            employee_id=row.employee_id,
            notification_type="recognition_review",
            title="认可已通过" if passed else "认可未通过",
            body=(
                f"{row.recognition_date} {row.recognition_type_name}认可已通过，计入 {Decimal(row.credited_fraction or 0):.1f} 分。{encouragement_message(record_id=row.id, recognition_type_name=row.recognition_type_name, content=row.content, stage='confirmed')}"
                if passed
                else f"{row.recognition_date} {row.recognition_type_name}认可未通过，请登录系统查看详情。"
            ),
            target_path="/",
            record_type="recognition",
            record_id=row.id,
    ...
    if notification:
        send_notification_pushes(db, notification.id)
    invalidate_data_caches()
    return {"ok": True, "record": recognition_payload(row), "notification_created": bool(notification)}
...
async def create_deduction(
    ...
    request_key = normalize_request_key(idempotency_key)
    duplicate_row = existing_submission(db, user.id, "deduction", request_key, DeductionRecord)
    if duplicate_row:
    ...
    try:
        ...
        remember_submission(db, user.id, "deduction", request_key, row.id)
...
async def create_sick_leave(
    ...
    request_key = normalize_request_key(idempotency_key)
    duplicate_row = existing_submission(db, user.id, "sick_leave", request_key, SickLeaveRecord)
    if duplicate_row:
    ...
    try:
        ...
        remember_submission(db, user.id, "sick_leave", request_key, row.id)
...
def resolve_governance_case(case_id: int, payload: dict, request: Request, db: Session = Depends(get_db), user: V2User = Depends(current_user)):
    ...
    notification = AppNotification(
        employee_id=row.submitted_by,
        notification_type="governance_case",
        title="申诉处理结果已生成",
        body="处理结论：" + ("维持原记录。" if decision == "uphold" else "需要按受控流程更正，请关注后续处理。"),
        target_path="/",
        record_type="governance_case",
        record_id=row.id,
    )
    db.add(notification)
...
def export_pr_rankings(
    ...
    if category == "overall":
        ...
        ws.append(headers)
        for row in data["rows"]:
            ws.append([row["rank"], row["employee_no"], row["employee_number_status"], row["employee_name"], row["role_name"], row["leader_name"], row["recognition_score"], row["deduction_score"], row["attendance_score"], row["total_score"]])
    elif category == "absence":
        headers = ["排名", "员工号", "工号状态", "姓名", "角色", "主管", "缺勤类型", "登记次数", "缺勤天数", "计费天数", "扣减全勤分", "最近一次缺勤"]
        ws.append(headers)
        for row in data["rows"]:
            ws.append([row["rank"], row["employee_no"], row["employee_number_status"], row["employee_name"], row["role_name"], row["leader_name"], data["subtype_name"], row["count"], row["leave_days"], row["charged_days"], row["score"], row["recent_date"]])
    else:
        ...
        ws.append(headers)
        for row in data["rows"]:
            ws.append([row["rank"], row["employee_no"], row["employee_number_status"], row["employee_name"], row["role_name"], row["leader_name"], data["subtype_name"], row["count"], row["score"], row["recent_date"]])
    style_sheet(ws)
    ...
    db.commit()
    filename = f"pr_rankings_{category}_{start_date}_{end_date}.xlsx"
...
def write_employee_number_history_sheet(wb: Workbook, db: Session, employee_ids: list[int]) -> None:
    ...
    for employee_id, changes in histories.items():
        ...
        for change in changes:
            ws.append([employee.employee_no, change.old_employee_no, employee.name, change.effective_on, "已变更", "历史记录与当前档案为同一员工"])
    if ws.max_row == 1:
        ...
        if kind == "attraction":
            ...
            for category in ("加分", "扣分", "缺勤", "全勤"):
                ...
                for item in category_rows:
                    ...
                    ws.append(["记录详情", item["date"], item["type"], item["object"], detail, item["add"], item["attendance"], item["deduction"], f"=SUM(F{ws.max_row + 1}:H{ws.max_row + 1})", context["attraction"], context["gsm"], context["leader"], str(employee_id), "明细", item["type"]])
...
def export_statistics(month: str, attraction_id: int | None = None, keyword: str | None = None, title: str | None = None, db: Session = Depends(get_db), user: V2User = Depends(require_permissions("DATA_EXPORT"))):
    ...
    ws0.title = "导出说明"
    ws0.append(["导出条件", "内容"])
    ws0.append(["统计月份", month])
    ws0.append(["景点圈", attraction_label])
    ws0.append(["Title", title_label])
    ws0.append(["员工搜索", keyword_label])
    ws0.append(["导出账号", f"{user.employee.employee_no} · {user.name}"])
    ws0.append(["导出账号角色", f"{user.role.name}（{user.role.code}）"])
    ws0.append(["导出时间", exported_at.strftime("%Y-%m-%d %H:%M:%S")])
    ws0.append(["在职计分人数", len(data["scores"])])
    ws0.append(["整月LOA人数", len(data["loa_rows"])])
    ws0.append(["小组平均分口径", f"{title_label}筛选后小组综合分 ÷ 筛选后人数"])
    style_sheet(ws0)
    ...
    # Open directly on the editable, collapsible hierarchy. The flat and original
    # tree sheets remain available for filtering and backwards-compatible review.
    ...
    for row in data["deductions"]:
        ...
        ws3.append([record_no, row["occurred_on"], row["current_employee_no"], row["employee_name"], row["deduction_type"], row["deduction_level"], row["points"], row["submitter_name"], row["status_name"], row["description"], upgrade_no, row.get("upgrade_role") or "", row.get("upgrade_state_name") or "", row["voided_by_name"], row["voided_at"], row["void_reason"], row["export_employee_no"], row["current_employee_no"], row["employee_number_status"]])
    ...
    for row in data["deductions"]:
        ...
        if not color and (row.get("upgrade_state") or "") == "pending": color = "FCE4D6"
        if not color and (row.get("upgrade_state") or "") == "rejected": color = "E7E6E6"
        if color:
            for cell in ws3[detail_row]: cell.fill = PatternFill("solid", fgColor=color)
    ...
    for row in data["sick_leaves"]:
        ...
        ws4.append([record_no, row["leave_start_date"], row["leave_end_date"], row["current_employee_no"], row["employee_name"], row["leave_days"], row["charged_days"], row["submitter_name"], row["status_name"], row["note"], row["voided_by_name"], row["voided_at"], row["void_reason"], row["export_employee_no"], row["current_employee_no"], row["employee_number_status"]])
    ...
    disposition = f'attachment; filename="{ascii_filename}"; filename*=UTF-8\'\'{quote(display_filename)}'
    return StreamingResponse(output, media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", headers={"Content-Disposition": disposition})
