...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\ops\Invoke-SqliteOnlineBackup.ps1 -RetentionDays 30 -DryRun
```

执行备份：

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\ops\Invoke-SqliteOnlineBackup.ps1 -RetentionDays 30
```

正式模式仅接受以下固定路径：

- 运行目录：`C:\Server\zhaojunjie\recognition-card-system`
- 数据库：`C:\Server\zhaojunjie\recognition-card-system\data_v2\recognition_v2.db`
- Python：`C:\Server\zhaojunjie\recognition-card-system\.venv\Scripts\python.exe`
...
powershell -NoProfile -ExecutionPolicy Bypass -File .\ops\Test-SqliteBackupRestore.ps1 -BackupFile $backup -DryRun
```

执行恢复演练：

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\ops\Test-SqliteBackupRestore.ps1 -BackupFile $backup
```

长期测试期可使用以下命令执行一次最新备份的月度演练：

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\ops\Run-MonthlyRestoreRehearsal.ps1
```
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\ops\Install-DailyBackupTask.ps1 -DailyAt "02:15" -RetentionDays 30 -DryRun
```

把整个 `ops` 目录放入正式应用目录后，以管理员 PowerShell 执行安装：

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File C:\Server\zhaojunjie\recognition-card-system\ops\Install-DailyBackupTask.ps1 -DailyAt "02:15" -RetentionDays 30
```
...
powershell -NoProfile -ExecutionPolicy Bypass -File .\ops\Invoke-SqliteOnlineBackup.ps1 `
...
```powershell
python .\ops\verify_readiness.py `
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\ops\Install-DailyBackupHealthTask.ps1 -DryRun
powershell -NoProfile -ExecutionPolicy Bypass -File .\ops\Install-DailyBackupHealthTask.ps1
```
...
- 运行隔离场景回归：`powershell -NoProfile -ExecutionPolicy Bypass -File .\ops\tests\Test-BackupHealthScenarios.ps1`。测试只在系统临时目录中生成模拟文件并在结束后清理，不读取生产数据库或生产备份。
