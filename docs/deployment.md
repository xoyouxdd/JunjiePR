...
推送到 `main` 时，GitHub Actions 工作流 [ci.yml](../.github/workflows/ci.yml) 只执行完整测试、版本号与最新更新记录一致性检查、文档本地链接检查和生产包构建；它不会自动改动生产服务器。CI 因为要跑测试，装的是 `backend/requirements-dev.txt`（继承生产依赖并加上 `pytest`、`httpx`），这不影响产物内容——正式包里始终只有生产依赖清单。构建成功后上传名为 `recognition-production-package-<完整提交号>` 的产物，保留 14 天。
