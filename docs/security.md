...
本系统仅公司内部使用。员工初始密码按工号规则生成（七位工号末四位）；管理员重置密码后，密码为登录账号后四位。两种场景均要求首次登录后修改密码。不要改成随机激活方案。口令长度策略保持现状，不要按公网标准加严。
...
发布包不得带上 `.env`、`.venv`、`secrets.json`、`data_v2`，也不带测试代码和测试依赖清单 `backend/requirements-dev.txt`，生产服务器因此不会装上 `pytest`、`httpx`。规则见 [release.md](release.md)。
