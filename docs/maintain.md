...
- 根目录 `README.md`：索引
- `docs/`：说明正文
- `server/ops/README.md`：备份脚本命令，跟脚本放在一起
...
| `server/ops/` 脚本 | `server/ops/README.md`，必要时改 `docs/operations.md` 一览表 |
| 密码/打包/仓库边界约定 | `docs/security.md` |
| `portal/` 是否还在用 | `docs/legacy-portal.md` |
| 用户能看见的功能 | `server/app/changelog.py`（见 [changelog.md](changelog.md)） |
...
- 不要把 `portal/` 写成现用前端
- 不要把初始密码规则写成「应改为随机激活」
- 不要复制 `ops/README.md` 的整份命令到 `docs/`，避免两处命令不一致
- 不要在文档里写尚未落地的功能
