...
用户已授权将设计成品上传 GitHub。三套 PPTX、原创图片素材及字段映射说明已归档至 [模板资料目录](assets/hr-monthly-templates/20260927/README.md)。从仓库下载源码时可一并取得；这些文件不进入当前生产运行包，也不代表系统的一键月报导出已实现。
...
现阶段提供本文档及设计成品资料。以上业务代码文件是接入计划，不是已实现文件名单。
