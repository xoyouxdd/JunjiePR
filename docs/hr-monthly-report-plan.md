...
阶段：三套可编辑模板已确认，已接入本地业务系统，纳入 `2026.09.27.2` 发布。GitHub 同步不代表生产部署，生产部署须另行授权。

三套设计成品已归档至 [模板资料目录](assets/hr-monthly-templates/20260927/README.md)。生产模板另存于`backend/app/report_templates/hr_monthly/`，由当前发布白名单纳入。

## 使用方式与接口

授权账号登录后，在数据分组打开“HR月报制作”。选择任意月份、任意单个景点圈或全部景点圈、模板，点击“预览报告”。按需勾选章节、优秀员工候选，并补充工作说明、生日祝福或活动照片后，点击“导出PPTX”。

- `GET /api/hr-monthly-reports/options`：模板、章节和可选景点圈。
- `GET /api/hr-monthly-reports/preview?month=YYYY-MM&attraction_id=...`：同口径报告数据和摘要指纹。
- `POST /api/hr-monthly-reports/export`：multipart `config`（JSON）和可选 `photos`，返回可编辑PPTX。参数含月份、范围、模板、章节、候选ID、文字和预览指纹。数据改变返回409，须重新预览。
- 四角色均具有专项 `HR_MONTHLY_REPORT` 权限，接口额外校验明确角色名单。其他登记、审核、作废权限不变。不新增业务表。
- 原生OOXML回填已确认模板，原生图表及内嵌工作簿、文字、表格保持可编辑。无数据、未确认优秀名单、无素材章节跳过；排名按原版容量自动分页。
- 活动照片只读取本次上传文件，不接受任意本地路径或远程附件URL。最多12张、每张5MB、总计30MB、单张1200万像素；导出时清除元数据并缩小到2000像素边长，完整适配不裁切。支持单图、双图、四图和多页。素材不持久化，不新增附件访问权。
- 同时最多两个生成任务，其余返回429。导出包含账号及时间水印，记录模板、范围、章节、确认名单、数据指纹和指标审计。导出包清除隐藏示例页、示例工作簿和示例备注。
- 生产使用Python标准库及现有openpyxl/Pillow依赖，无须Office、Node或Codex。
...
| `backend/tests/test_hr_monthly_reports.py`（同一模块） | 原生对象、分页、中文、图片比例、无素材跳页 |
| `scripts/build_release.py`、依赖清单 | 将模板和实际所需生成依赖纳入包，不能依赖开发电脑的Codex运行时 |
| `backend/app/changelog.py`及说明文档 | 接入完成后才声明功能可用；发布公告选择保持原规则 |

实际新增`hr_monthly_report.py`、`hr_monthly_pptx.py`、`routers/hr_monthly_reports.py`、三套PPTX和`tests/test_hr_monthly_reports.py`；修改路由汇总、权限种子、页面JS/CSS、更新记录和文档索引。现有发布规则已覆盖模板，无额外运行时依赖。
...
| 认可发放人 | `RecognitionRecord.recognizer_employee_id`和签卡人快照 | 按签卡人统计发放数量，代录人不重复累加 |
...
本地验证：全量49个独立测试模块通过；月报专项覆盖明确角色权限、跨圈查询、预览指纹、三套包结构、内嵌图表工作簿、19人自动分页、照片完整适配、LOA排除、历史归属及实际计入分值。三套合成测试报告成功导入渲染工具，页面也已通过浏览器桌面及390px视口检查。未在桌面PowerPoint中检查，不将本地验证描述为生产上线。
